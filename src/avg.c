...
static float8
avg_integral_real(float8 N, float8 L, Complex *val, int32 t1, int32 t2)
{
  float8 res;
  float8 mid = L / 2;
  float8 period = t2 - t1;
...
static float8
avg_integral_imag(float8 N, float8 L, Complex *val, int32 t1, int32 t2)
{
  float8 res;
  float8 mid = L / 2;
  float8 period = t2 - t1;
...
Datum
complex_avg_accum(PG_FUNCTION_ARGS)
{
  ...
  transvalues = check_float8_array(transarray, "complex_avg_accum", 4);
  N = transvalues[0];
  Sx = transvalues[1];
  Sy = transvalues[2];
  ...
  else
  {
    ...
    result = construct_array(transdatums, 4, FLOAT8OID,
